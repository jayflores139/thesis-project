      ...
      if (isset($_GET['rid'])) {
        ...
        if (mysqli_num_rows($query) > 0) {
          ...
          <fieldset style="margin-top:10px;margin:5px" class="w3-border-0 w3-center">
            ...
            if ($q['r_status'] == "pending" || ($q['r_status'] == "approve") || ($q['r_status'] == "finish") ) { ?>
            <input type="hidden" id="cancel_id_r<?php echo $q['rid'] ?>" value="<?php echo $q['rid'] ?>">
            <button class="btn btn-danger cancel_r w3-round w3-padding" id="<?php echo $q['rid'] ?>">Cancel</button>
           <?php
            }
            ?>
            <?php
            ...
            if ($q['r_status'] == "pending" || $q['r_status'] == "approve") {
              echo '<button class="btn btn-default w3-margin w3-border w3-padding w3-round dialog-link">Add Payment</button>';
            }
            ?>
...
<!-- Payment Form ###################################### -->
              <p>
                ...
                if ($q['r_status'] == "pending") { ?>
                  ...
                  <label class="w3-margin-top w3-left" style="font-weight:normal;">Downpayment</label>
                  <input value="<?php echo $q['payable'] - ($q['payable'] * $downs['d_price']) ?>" type="number" class="w3-input w3-border w3-round w3-margin-bottom" id="pending_input">
                  <input type="hidden" id="pending_input_hidden" value="<?php echo $q['rid'] ?>">
                  <button id="pending_btn" class="w3-btn w3-light-grey w3-border w3-round">Submit</button>

                 <?php
                } elseif ($q['r_status'] == "approve") { ?>

                  <label class="w3-margin-top w3-left" style="font-weight:normal;">Full Payment</label>
                  <input id="approve_input" value="<?php echo $q['balance'] ?>" type="number" class="w3-input w3-border w3-round w3-margin-bottom">
                  <input type="hidden" id="approve_input_hidden" value="<?php echo $q['rid'] ?>">
                  <button id="approve_btn" class="w3-btn w3-light-grey w3-border w3-round">Submit</button>
      ...
      if (mysqli_num_rows($e) > 0) {
        ...
        $r = mysqli_query($conn,"SELECT * FROM food_type where cat_id = '".$ee['cat_id']."'") or die(mysqli_error($conn));
          while ($rr = mysqli_fetch_array($r)) {
            ...
            $t = mysqli_query($conn,"SELECT food_menu.food_name FROM food_menu inner join custom_r where food_menu.food_id=custom_r.food_id and type_id = '".$rr['type_id']."' and r_id = '$rid'") or die(mysqli_error($conn));
