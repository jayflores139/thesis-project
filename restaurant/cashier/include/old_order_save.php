...
include '../../includes/connect.php';
	...
	if (isset($_POST['order_id_save'])) {
		...
		$oder_amount = $overAllTotal + ($overAllTotal * .12);
		...
		if ($ff['order_type'] == "delivery") {
			...
			elseif ($ff['customer_type'] == "junior") {
				...
				$amountWithDeliveryCharge = $overAllTotal + ($overAllTotal * .12) + $ww['deliv_charge'];
			...
			elseif ($ff['customer_type'] == "junior") {
				$amount = $overAllTotal + ($overAllTotal * .12);
	...
	if (isset($_POST['order_id_cancel'])) {
		...
		if ($old_Order == true) {
			...
			$overAmount = $order_amount + ($order_amount * .12);
			...
			if ($rr['order_type'] == "delivery") {
				...
				elseif ($rr['customer_type'] == "junior") {
					...
					$amountWithDeliveryCharge = $order_amount + ($order_amount * .12) + $ww['deliv_charge'];
				...
				elseif ($rr['customer_type'] == "junior") {
					$amount = $order_amount + ($order_amount * .12);
