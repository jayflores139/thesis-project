  ...
  if ($transaction_date < $today) {
    ...
    if (!empty($delivery_personName) || !empty($delivery_personContact)) {
        ...
        if ($query == true) {
          ...
          mysqli_query($conn,"INSERT INTO pick_up_details (p_to_pick, contact, order_id, type) VALUES ('$delivery_personName', '$delivery_personContact', '$last_id', 'walk in') ") or die(mysqli_error($conn));
        }
        if (!empty($_SESSION['shopping_cart'])) {
          ...
          $ddd = mysqli_fetch_array($dd);
            ...
            $orderAmountWithDiscount = ($totalS - ($totalS * $seniorDiscount)) + $ddd['deliv_charge'];
  ...
  if ($transaction_date < $today) {
    $data['invalid'] = "Invalid dine-in date!";
        ...
        if ($query == true) {
          ...
          mysqli_query($conn,"INSERT INTO pick_up_details (p_to_pick, contact, order_id, type) VALUES ('$person_NAME', '$contact_person', '$last_id', 'walk in') ") or die(mysqli_error($conn));
        }
        if (!empty($_SESSION['shopping_cart'])) {
          $totalS = 0;
          foreach ($_SESSION['shopping_cart'] as $key => $value) {
    ...
    $data['invalid'] = "Invalid dine-in date!";
        ...
        if (!empty($_SESSION['shopping_cart'])) {
          ...
          foreach ($_SESSION['shopping_cart'] as $key => $value) {
            ...
            $orderAmount = $totalS - ($totalS * $seniorDiscount);
